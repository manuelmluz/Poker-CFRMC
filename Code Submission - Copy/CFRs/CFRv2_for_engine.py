...
# 
class CFREngineV2(BasePokerPlayer):  # Do not forget to make parent class as "BasePokerPlayer"
...
# BLUEPRINT FUNCTIONS:    
    ...
    def select_action_from_strategy(strategy):
        ...
        if strategy is None:
            return CFREngineV2.no_information_set_strategy()
        ...
        try:          
            ...
            return CFREngineV2.no_information_set_strategy()  # Fallback to default
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        raw_information_set = CFREngineV2.get_information_set_engine(round_state, hole_card, community_cards)
        #abstract it using the abstraction used in training 
        abstracted_information_set = CFREngineV2.information_set_to_abstracted_set(raw_information_set)
        #get the strategy looks like [0.9,0.1,0.1]
        strategy = CFREngineV2.get_strategy(df,abstracted_information_set)
        #select the specific action
        selected_action = CFREngineV2.select_action_from_strategy(strategy)
        #print(selected_action)

        if selected_action == "not blueprint":
            #use monte carlo
            win_rate = estimate_win_rate(10000, self.players_remaining, hole_card, round_state['community_card'])
            ...
            #use blueprint
            action, amount = CFREngineV2.action_to_engine(selected_action, valid_actions, round_state)
